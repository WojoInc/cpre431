...
function check(data_array, cache_size)
{
    ...
    // Use Blob to create a URL to the script that needs to be loaded.
    // This is used as a workaround to allow for loading WebWorker scripts from disk, which is normally blocked
    // for obvious reasons
    ...
    // Convert size in MB to actual number of bytes
...
const CACHE_SIZE = [8,16,32,64,128];
...
function main()
{
    ...
    {
        for (let i = 0; i < CACHE_SIZE.length; i++) {
            log("Testing with a cache size of: " + CACHE_SIZE[i] + "MB");
            check([115, 112, 101, 99, 116, 114, 101, 46, 106, 115], CACHE_SIZE[i]);
        }
...
if(window.SharedArrayBuffer){
    ...
    banner.innerHTML = "<div style='color:greenyellow'><center><h1>SharedArrayBuffer is available!</h1></center></div>"
}
else{
    let banner = document.getElementById("banner");
    banner.innerHTML = "<div style='color:red'><center><h1>SharedArrayBuffer is not available!</h1></center></div>"
}